...
// ManagerError is this package's error type — same pattern as task.TaskError
// and worker.WorkerError. The duplication keeps each package self-contained,
// and the name says which layer an error came from.
type ManagerError struct {
	Op      string // e.g. "manager.SendWork"
...
func E(op, message string, err error) *ManagerError {
	return &ManagerError{Op: op, Message: message, Err: err, Stack: string(debug.Stack())}
}
...
func Wrap(op, message string, err error) *ManagerError {
	return &ManagerError{Op: op, Message: message, Err: err}
}
